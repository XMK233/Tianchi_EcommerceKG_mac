...
class TransD(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim):
        ...
        # 结构向量 (用于构建投影矩阵)
        self.entity_proj_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_proj_embeddings = nn.Embedding(num_relations, embedding_dim)
        nn.init.xavier_uniform_(self.entity_proj_embeddings.weight)
        nn.init.xavier_uniform_(self.relation_proj_embeddings.weight)
    
    def _build_projection_matrix(self, e_p, r_p):
        # 构建投影矩阵 M = r_p * e_p^T + I
        outer_product = torch.bmm(r_p.unsqueeze(2), e_p.unsqueeze(1))  # [batch_size, embedding_dim, embedding_dim]
        identity = torch.eye(self.embedding_dim, device=e_p.device).unsqueeze(0).expand(e_p.size(0), -1, -1)
        return outer_product + identity
    ...
    def forward(self, h_ids, r_ids, t_ids, t_neg_ids=None):
        ...
        # 获取结构向量
        h_p = self.entity_proj_embeddings(h_ids)  # [batch_size, embedding_dim]
        r_p = self.relation_proj_embeddings(r_ids)  # [batch_size, embedding_dim]
        t_p = self.entity_proj_embeddings(t_ids)  # [batch_size, embedding_dim]
        
        # 构建头实体和尾实体的投影矩阵
        M_hr = self._build_projection_matrix(h_p, r_p)  # [batch_size, embedding_dim, embedding_dim]
        M_tr = self._build_projection_matrix(t_p, r_p)  # [batch_size, embedding_dim, embedding_dim]
        
        # 投影实体
        h_proj = torch.bmm(M_hr, h.unsqueeze(2)).squeeze(2)  # [batch_size, embedding_dim]
        t_proj = torch.bmm(M_tr, t.unsqueeze(2)).squeeze(2)  # [batch_size, embedding_dim]
        
        # 计算正样本得分
        pos_score = torch.norm(h_proj + r - t_proj, p=1, dim=1)
        ...
        if t_neg_ids is not None:
            ...
            # 扩展头实体和关系的结构向量
            h_p_expanded = h_p.unsqueeze(1).expand(-1, neg_samples, -1).contiguous().view(-1, self.embedding_dim)
            r_p_expanded = r_p.unsqueeze(1).expand(-1, neg_samples, -1).contiguous().view(-1, self.embedding_dim)
            
            # 获取负样本尾实体的语义和结构向量
            t_neg_ids_flat = t_neg_ids.view(-1)
            t_neg = self.entity_embeddings(t_neg_ids_flat)  # [batch_size*neg_samples, embedding_dim]
            t_neg_p = self.entity_proj_embeddings(t_neg_ids_flat)  # [batch_size*neg_samples, embedding_dim]
            
            # 为负样本构建投影矩阵
            M_tr_neg = self._build_projection_matrix(t_neg_p, r_p_expanded)  # [batch_size*neg_samples, embedding_dim, embedding_dim]
            
            # 投影负样本尾实体
            t_neg_proj = torch.bmm(M_tr_neg, t_neg.unsqueeze(2)).squeeze(2)  # [batch_size*neg_samples, embedding_dim]
            
            # 计算负样本得分
            neg_score = torch.norm(h_expanded + r_expanded - t_neg_proj, p=1, dim=1)  # [batch_size*neg_samples]
    ...
    def normalize_embeddings(self):
        ...
        with torch.no_grad():
            ...
            self.relation_embeddings.weight.div_(torch.norm(self.relation_embeddings.weight, dim=1, keepdim=True))
...
def evaluate(model, dev_dataset, mapper, device, batch_size=128, max_entities_per_batch=4000):
    ...
    with torch.no_grad():
        for batch_start in range(0, total_dev_triples, batch_size):
            ...
            # 获取语义向量
            h = model.entity_embeddings(h_ids)
            r = model.relation_embeddings(r_ids)
            t = model.entity_embeddings(t_ids)
            
            # 获取结构向量
            h_p = model.entity_proj_embeddings(h_ids)
            r_p = model.relation_proj_embeddings(r_ids)
            t_p = model.entity_proj_embeddings(t_ids)
            
            # 构建头实体的投影矩阵并投影
            M_hr = model._build_projection_matrix(h_p, r_p)
            h_proj = torch.bmm(M_hr, h.unsqueeze(2)).squeeze(2)
            ...
            for entity_start in range(0, mapper.entity_count, max_entities_per_batch):
                ...
                t_neg = model.entity_embeddings(entity_ids)
                t_neg_p = model.entity_proj_embeddings(entity_ids)
                
                # 扩展以匹配批次大小
                t_neg_expanded = t_neg.unsqueeze(0).expand(current_batch_size, -1, -1)
                t_neg_p_expanded = t_neg_p.unsqueeze(0).expand(current_batch_size, -1, -1)
                r_p_expanded = r_p.unsqueeze(1).expand(-1, entity_chunk_size, -1)
                
                # 为当前块构建投影矩阵
                M_tr_chunk = model._build_projection_matrix(t_neg_p_expanded.reshape(-1, model.embedding_dim), 
                                                          r_p_expanded.reshape(-1, model.embedding_dim))
                M_tr_chunk = M_tr_chunk.reshape(current_batch_size, entity_chunk_size, model.embedding_dim, model.embedding_dim)
                
                # 投影负样本尾实体
                t_neg_proj = torch.einsum('bijk,bik->bij', M_tr_chunk, t_neg_expanded)
                
                # 计算得分
                h_plus_r_expanded = h_plus_r.unsqueeze(1)
                chunk_scores = torch.norm(h_plus_r_expanded - t_neg_proj, p=1, dim=2)
    ...
    with torch.no_grad():
        for batch_start in range(0, process_count, batch_size):
            ...
            for entity_start in range(0, mapper.entity_count, max_entities_per_batch):
                ...
                t_neg_expanded = t_neg.unsqueeze(0).expand(batch_size_current, -1, -1)
                t_neg_p_expanded = t_neg_p.unsqueeze(0).expand(batch_size_current, -1, -1)
                r_p_expanded = r_p.unsqueeze(1).expand(-1, entity_chunk_size, -1)
                
                M_tr_chunk = model._build_projection_matrix(t_neg_p_expanded.reshape(-1, model.embedding_dim), 
                                                          r_p_expanded.reshape(-1, model.embedding_dim))
                M_tr_chunk = M_tr_chunk.reshape(batch_size_current, entity_chunk_size, model.embedding_dim, model.embedding_dim)
...
def main():
    ...
    print("正在创建TransD模型...")
    model = TransD(mapper.entity_count, mapper.relation_count, EMBEDDING_DIM)
    model.to(device)
