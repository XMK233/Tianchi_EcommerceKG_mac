...
# # 优先使用项目内的数据集路径
# TRAIN_FILE_PATH = os.path.join(PROJECT_ROOT, "originalData", "OpenBG500", "OpenBG500_train.tsv")
# TEST_FILE_PATH = os.path.join(PROJECT_ROOT, "originalData", "OpenBG500", "OpenBG500_test.tsv")
# DEV_FILE_PATH = os.path.join(PROJECT_ROOT, "originalData", "OpenBG500", "OpenBG500_dev.tsv")
# OUTPUT_FILE_PATH = os.path.join(PROJECT_ROOT, "preprocessedData", "OpenBG500_test.tsv")

# # 如果项目内路径不存在，回退到用户指定的路径
# if not os.path.exists(TRAIN_FILE_PATH):
#     print(f"警告：项目内未找到训练数据文件，尝试使用备用路径...")
...
MAX_HEAD_ENTITIES = 100  # 设置一个合理的默认值，确保至少处理一些实体
...
class KnowledgeGraphDataset(Dataset):
    def __init__(self, file_path, is_test=False, is_dev=False, max_lines=None):
        ...
        try:
            ...
            print(f"成功加载 {file_path}，共 {len(self.triples)} 条数据")
...
def predict_tail_entities(model, test_dataset, mapper, device, max_head_entities=None, batch_size=128, max_entities_per_batch=10000):
    ...
    # 添加对空数据集的检查
    if total_test_triples == 0:
        print("警告：测试数据集为空，无法进行预测！")
        print(f"检查文件路径: {TEST_FILE_PATH}")
        return
    
    # 确定要处理的头实体数量
    # 修复：确保max_head_entities有合理的值
    if max_head_entities is None:
        max_head_entities = total_test_triples
    process_count = min(max_head_entities, total_test_triples)
    
    print(f"\n开始预测尾实体...")
    print(f"测试数据集大小: {total_test_triples}")
