...
class TransE(nn.Module):
    ...
    def forward(self, h_ids, r_ids, t_ids, t_neg_ids=None):
        ...
        # 计算正样本得分：||h + r - t||_1 或 ||h + r - t||_2
        pos_score = torch.norm(h + r - t, p=1, dim=1)  # [batch_size]
        ...
        if t_neg_ids is not None:
            ...
            # 扩展头实体和关系嵌入以匹配负样本数量
            h_expanded = h.unsqueeze(1).expand(-1, neg_samples, -1).contiguous().view(-1, self.embedding_dim)  # [batch_size*neg_samples, embedding_dim]
    ...
    def normalize_relations(self):
        # 归一化关系嵌入（TransE通常也会归一化关系嵌入）
        with torch.no_grad():
            norms = torch.norm(self.relation_embeddings.weight, dim=1, keepdim=True)
            norms = torch.maximum(norms, torch.tensor(1e-12, device=norms.device))
            self.relation_embeddings.weight.div_(norms)
...
def train_model(model, train_dataset, mapper, device, epochs=EPOCHS, batch_size=BATCH_SIZE):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 归一化实体嵌入和关系嵌入（TransE通常需要归一化关系嵌入）
            model.normalize_entities()
            model.normalize_relations()
    ...
    with torch.no_grad():
        ...
        for batch_start in range(0, total_dev_triples, batch_size):
            ...
            for entity_start in range(0, mapper.entity_count, max_entities_per_batch):
                ...
                entity_embeddings_reshaped = entity_embeddings.view(1, entity_chunk_size, -1).expand(current_batch_size, -1, -1)
                chunk_scores = torch.norm(
                    h_plus_r_expanded - entity_embeddings_reshaped, 
                    p=1, 
                    dim=2
                ...
                # 清理中间变量，释放内存
                del entity_embeddings, entity_embeddings_reshaped, chunk_scores
                clear_memory(device)
            ...
            mrr_score += torch.sum(1.0 / ranks.float()).item()
            total_count += current_batch_size
            
            # 显示进度
            progress = total_count / total_dev_triples * 100
            print(f"  评估进度: {progress:.1f}% ({total_count}/{total_dev_triples})", end='\r')
...
def predict_tail_entities(model, test_dataset, mapper, device, output_file_path, max_head_entities=None, batch_size=128, max_entities_per_batch=2000):
    model.eval()
    
    # 如果设置了max_head_entities，则只处理指定数量的头实体
    if max_head_entities is not None and max_head_entities > 0:
        process_count = min(max_head_entities, len(test_dataset))
        test_triples = test_dataset.triples[:process_count]
    else:
        process_count = len(test_dataset)
        test_triples = test_dataset.triples
    
    print(f"\n开始预测尾实体...")
    print(f"将处理 {process_count} 个测试三元组")
    
    # 用于存储结果的列表
    results = []
    results_buffer = []
    buffer_size = 1000  # 每1000个结果写入一次内存
    ...
    with torch.no_grad():
        ...
        for batch_start in range(0, process_count, batch_size):
            ...
            # 为每个查询获取得分最高的前10个实体（得分越小越好）
            top_scores, top_indices = torch.topk(batch_scores, k=10, dim=1, largest=False)
            
            # 将结果转换为原始实体ID
            for i in range(current_batch_size):
                h, r, _ = batch_triples[i]
                top_entities = [mapper.id_to_entity[idx.item()] for idx in top_indices[i]]
                # 按照要求的格式生成输出行
                result_line = f"{h}\t{r}\t{','.join(top_entities)}"
                results_buffer.append(result_line)
                
                # 当缓冲区达到指定大小时，将结果添加到主列表并清空缓冲区
    ...
    try:
        ...
        print(f"压缩后的结果已保存到 {zip_file_path}")
    except ImportError:
        print("未找到zipfile模块，跳过压缩步骤")
...
def main():
    ...
    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
        ...
        print("使用MPS设备加速计算")
        # MPS设备预热，避免首次计算较慢
        torch.ones((100, 100), device=device).sum()
        # 清理预热产生的内存
        clear_memory(device)
    elif torch.cuda.is_available():
        device = torch.device('cuda')
        print(f"使用CUDA设备 {torch.cuda.get_device_name(0)} 加速计算")
    else:
        device = torch.device('cpu')
        print("使用CPU进行计算")
        # 当使用CPU时，减小批次大小以避免内存问题
        global BATCH_SIZE
        BATCH_SIZE = 64
        global ENTITY_BATCH_SIZE
    ...
    print(f"开发集大小: {len(dev_dataset)}")
    print(f"测试集大小: {len(test_dataset)}")
    
    # 构建实体和关系映射
    mapper = EntityRelationMapper()
    mapper.build_mappings(train_dataset, dev_dataset, test_dataset)
